...
function renderSubjects(cls) {
  ...
  if (title) {
    title.textContent = `Class ${cls} — choose a subject`;
  }
  ...
  list.innerHTML = subjects.length
        ...
        >
          ...
          <small>
            ${
              notes.filter(
                n =>
                  String(n.class).toUpperCase() === cls &&
    ...
    : `
        <div class="empty">
          No subjects have been added for Class ${esc(cls)} yet.
        </div>
      `;

  list.querySelectorAll('[data-subject]').forEach(btn => {
    btn.onclick = () => {
      renderNotesForSubject(btn.dataset.subject);
    };
  });
    ...
    ? filtered.map(n => `
        <article class="resource">
          ...
          <a
            href="${esc(
              n.fileUrl ||
              `/api/view-note?id=${encodeURIComponent(n.id)}`
            )}"
            target="_blank"
            rel="noopener"
          >
...
async function deleteNote(id) {
  ...
  try {
    const r = await fetch(
      `/api/delete-note?id=${encodeURIComponent(id)}`,
      {
        method: 'DELETE',
        headers: {
