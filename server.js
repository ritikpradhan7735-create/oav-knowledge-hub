    ...
    try {
      ...
      const result =
        await cloudinary.search
          .expression(
            'resource_type:raw AND folder:oav_hub_pdf_notes'
          )
          .with_field('context')
  ...
  async (req, res) => {
    ...
    if (!adminOk(req.body)) {
      ...
      return res.status(401).json({
        ...
        message:
          'Invalid admin credentials. Set ADMIN_USER and ADMIN_PASS in Render Environment.'
    ...
    const note = {
      ...
      class:
        req.body.classNum || 'IX',

      subject:
        (req.body.subject || 'General').trim(),
    ...
    try {
      ...
      if (cloudReady()) {
        ...
        // Save the Cloudinary-generated values
  ...
  async (req, res) => {
    ...
    if (!key) {
      ...
      return res.json({
        ...
        reply:
          'Study Buddy AI is not configured yet. Add GEMINI_API_KEY in Render Environment Variables.'
    ...
    try {
...

                  {
                    ...
                    parts: [

                      {
                        ...
                        text:
                          `You are Study Buddy for Indian school students. Give accurate, concise, easy-to-understand educational help. Question: ${prompt}`
      ...
      res.status(500).json({
        ...
        reply:
          'Unable to reach Study Buddy right now.'
...
server.listen(
  ...
  () => {
    ...
    console.log(
      `🚀 OAV Hub server running on port ${PORT}`
    );
